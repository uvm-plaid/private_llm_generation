...
if __name__ == "__main__":
    ...
    # root_dir = Path("./raw/")
    # root_dir = Path("/users/k/n/kngongiv/Research/impossible_kd/SummKD-submission/stage1/summary/gen_data/gpt2-xl/")
    root_dir = Path("/users/k/n/kngongiv/Research/impossible_kd/SummKD-submission/stage1/summary/gen_data/BioGPT-large/")
    ...
    out_filename = Path("processed/all.jsonl")
    print("\nOut Filename: ", out_filename)
    save_to_file([pair.to_dict() for pair in all_out_pairs], out_filename)

    # input_file_path = 'all.jsonl'
    train_file_path = Path("processed/train.jsonl")
    valid_file_path = Path("processed/valid.jsonl")
