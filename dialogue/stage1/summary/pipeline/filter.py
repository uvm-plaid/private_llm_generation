...
class ConFilter:
    def __init__(self, args: Namespace):
        ...
        self.length_threshold = 0.8
        self.nli_threshold = 0.9
        self.reverse_nli_threshold = 0.6
        self.rougeL_threshold = 0.6
    ...
    def filter_bootstrap(self, sample: Candidates) -> List[Pair]:
        ...
        # y_con_pair_indices = itertools.permutations(range(len(y_cons)), 2)  # (y_orig, y_summ)
        ...
        # # Create pairs of y_cons
        y_con_pair_indices = itertools.permutations(range(len(y_cons)), 2)  # (y_orig, y_summ)  # REPLACED y_new with y_summ
        y_con_pairs = [Pair(x_l=x_l, y_orig=y_cons[idx1], y_summ=y_cons[idx2], y_orig_idx=idx1, y_summ_idx=idx2)
                    for idx1, idx2 in y_con_pair_indices]
    ...
    def set_and_filter_nli(self, pairs: List[Pair]) -> List[Pair]:
        ...
        y_summ_list = [pair.y_summ for pair in pairs]  # REPLACED y_new with y_summ
    ...
    def set_and_filter_reverse_nli(self, pairs: List[Pair]) -> List[Pair]:
        ...
        reverse_NLI_scores = prediction[:, 2].tolist()
    ...
    def filter_length(self, pair: Pair) -> bool:
        # return len(pair.y_new) <= self.length_threshold * len(pair.y_orig)
         return len(pair.y_summ) <= self.length_threshold * len(pair.y_orig) # REPLACED y_new with y_summ
    ...
    def filter_number(self, pairs: List[Pair]) -> List[Pair]:
        out_pairs = []
        for pair in pairs:
            y_summ_numbers = self.numerize(pair.y_summ) #y_new) # REPLACED y_new with y_summ

            if len(y_summ_numbers) == 0:
                out_pairs.append(pair)
            else:
                ...
                all_included = all([str(key) in y_orig_numbers or str(value) in y_orig_numbers
