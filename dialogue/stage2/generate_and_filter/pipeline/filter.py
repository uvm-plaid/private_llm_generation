...
class ConFilter:
    ...
    def filter_finetuned(self, sample: Candidates) -> List[Pair]:
        ...
        y_con_pairs = [Pair(x_l=x_l, y_orig=y_orig, y_summ=y_con, y_orig_idx=0, y_summ_idx=idx)
        ...
        if len(y_con_pairs) > 0:
            y_con_pairs = [min(y_con_pairs, key=lambda x: x.reverse_nli_score)]
        else:
            y_con_pairs = []
    ...
    def filter_length(self, pair: Pair) -> bool:
        # return len(pair.y_new) <= self.length_threshold * len(pair.y_orig) # REPLACED y_new with y_summ
        return len(pair.y_summ) <= self.length_threshold * len(pair.y_orig)
