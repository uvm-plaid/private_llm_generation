...
def main(dpsgd_file, ours_file, output_file, save_file):
    ...
    dpsgd_added_records = []
    dpsgd_dropped_records = []
    ours_added_records = []
    ours_dropped_records = []

    for idx, (instr, dpsgd_out, ours_out) in enumerate(zip(pii_instructions, pii_dpsgd_outputs, pii_ours_outputs)):
        added_dpsgd, dropped_dpsgd = compute_pii_changes_entailment(instr, dpsgd_out) #compute_pii_changes(instr, dpsgd_out)
        added_ours, dropped_ours = compute_pii_changes_entailment(instr, ours_out) #compute_pii_changes(instr, ours_out)
        
        for added in added_dpsgd:
            dpsgd_added_records.append({
                "Index": idx,
                "Original Example": instructions[idx],
                "Original PII Values": [item['text'] for item in instr],
                "DPSGD Output": dpsgd_outputs[idx],
                "DPSGD PII Values": [item['text'] for item in dpsgd_out],
                "Added PII": added
            })
        
        for dropped in dropped_dpsgd:
            dpsgd_dropped_records.append({
                "Index": idx,
                "Original Example": instructions[idx],
                "Original PII Values": [item['text'] for item in instr],
                "DPSGD Output": dpsgd_outputs[idx],
    ...
    path = "/users/k/n/kngongiv/Research/private_llm_generation/dialog/evaluation/hallucination_results/mrpc"
    pd.DataFrame(dpsgd_added_records).to_csv(f"{path}/dpsgd_pii_added_2.csv", index=False)
    pd.DataFrame(dpsgd_dropped_records).to_csv(f"{path}/dpsgd_pii_dropped_2.csv", index=False)
    ...
    freq_data = pd.DataFrame({
        ...
        "Count": [len(dpsgd_added_records), len(dpsgd_dropped_records), len(ours_added_records), len(ours_dropped_records)]
    })
...
#     # Test setup for fixed file paths
#     dpsgd_file = '/users/k/n/kngongiv/Research/private_llm_generation/dialog/evaluation/alpaca_eval/data/dp_mrpc.json'
#     ours_file = '/users/k/n/kngongiv/Research/private_llm_generation/dialog/evaluation/alpaca_eval/data/impossible_mrpc.json'
#     output_file = 'hallucinations_mrpc.pdf'
#     save_file = '/users/k/n/kngongiv/Research/private_llm_generation/dialog/evaluation/hallucination_results/mrpc_pii.json'
#     main(dpsgd_file, ours_file, output_file, save_file)
