...
def main(args):
    ...
    # wandb.init(project="Paraphrase_Dataset", entity="ivolinengong", name=args.run_name)
    ...
    elif args.dataset_name == "biosses":
    ...
    tokenized_train_dataset = train_dataset.map(lambda examples: preprocess_function(examples, tokenizer, args.max_input_length, args.max_output_length), batched=True)
    ...
    if model_checkpoint:
        ...
        model = T5ForConditionalGeneration.from_pretrained(args.student_model_dir).to(device)
        tokenizer = T5Tokenizer.from_pretrained(args.model_name)
        optimizer = AdamW(model.parameters(), lr=args.learning_rate)
        best_loss = float('inf')
    ...
    for epoch in range(args.epochs):
        ...
        for i, batch in enumerate(data_loader):
            ...
            avg_loss = per_example_loss.mean().item()
            total_loss += avg_loss * len(per_example_loss)  # Adjust for the batch size
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project_name", type=str, default="DP_Paraphrase", help="Name of the WandB project.")
    parser.add_argument("--student_model_dir", type=str, default="", help="Name of the model to use.")
    ...
    parser.add_argument("--dataset_name", type=str, required=True, choices=["mrpc", "biosses"], help="dataset_name name.")
    parser.add_argument("--max_input_length", type=int, default=128, help="Maximum input length for the tokenizer.")
    parser.add_argument("--max_output_length", type=int, default=128, help="Maximum output length for the tokenizer.")
